...
public class TextExtractor {
		...
		private static List<InlineItem> crawlStructure(Node n) {
			...
			items.add(new InlineString("",lsSerializer.writeToString(n)));	
			return items;
		}
		...
		private static RASETag produceTag(Element element) {
			...
			if (type!=null && property!=null ) {
				RASETag tag = new RASETag(type,property,element.getAttribute("data-raseComparator"),element.getAttribute("data-raseTarget"),element.getAttribute("data-raseUnit"),element.getAttribute("id"),innerXml(element));
				return tag;
			}
			return null;
		}
